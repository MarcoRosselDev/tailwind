    ...
    async function call_end() {
      try {
        const call = await fetch(
          "https://jsonplaceholder.typicode.com/todos/1712312312"
        );
        if (!call.ok) {
          throw new Error("Error de la api fake api");
        }
        const data = (await call.json()) as FakeApi; // asi le decimos que cuando se convierta en json se comporte como el type FakeApi
        setData(data);
      } catch (err: unknown) {
        if (err instanceof Error) {
          console.log({ message: err.message });
